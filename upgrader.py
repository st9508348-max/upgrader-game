...
<head>
	...
	<style>
		...
		*{box-sizing:border-box} body{margin:0;background:var(--bg);color:var(--text);font:14px Arial,sans-serif;min-width:320px}
		...
		.outcome{position:fixed;inset:0;z-index:20;display:none;place-items:center;padding:22px;background:#111318e8}.outcome.show{display:grid}.outcome-card{width:min(620px,100%);border:2px solid var(--cyan);background:var(--panel);padding:42px 28px;text-align:center;box-shadow:0 0 80px #67e4d733}.outcome-card.loss{border-color:var(--orange)}.outcome-card h2{margin:0;color:var(--cyan);font-size:clamp(48px,12vw,108px);line-height:.85;letter-spacing:2px}.outcome-card.loss h2{color:var(--orange)}.outcome-card p{color:var(--muted);font-size:14px;margin:22px 0}.outcome-card button{width:auto;margin:0;padding:12px 28px}
	</style>
</head>
<body>
...
<section class="outcome" id="outcome" aria-live="assertive"><div class="outcome-card" id="outcomeCard"><h2 id="outcomeTitle">UPGRADE WON</h2><p id="outcomeText"></p><button class="button" id="closeOutcome" type="button">Continue</button></div></section>
<div class="wrap">
	...
	<section class="game">
		<div class="panel"><div class="label">01 / INVENTORY</div><h2>Your weapons <small id="inventoryCount">10 / 10 owned</small></h2><div class="cards" id="inputs"></div></div>
...
const starterNames=['Ash Circuit','Iron Phantom','Chrome Haze','Silver Viper','Frostline','Dust Runner','Slate Nova','Steel Echo','Moonlit Alloy','Ghost Metal'];
const starterWeapons=Array.from({length:10},(_,i)=>({name:starterNames[i],type:'STARTER / GRAY',value:100,a:'#c4c9cf',b:'#5d6670',bg:'#252a30'}));
const targets=[{name:'Chrome Drift',type:'PISTOL / RARE',value:200,a:'#d5d9df',b:'#657080',bg:'#252a30'},{name:'Steel Pulse',type:'SMG / RARE',value:300,a:'#8fd8e8',b:'#315d78',bg:'#172b35'},{name:'Violet Static',type:'RIFLE / CLASSIFIED',value:620,a:'#d379ff',b:'#3434a4',bg:'#24203b'},{name:'Carbon Bloom',type:'KNIFE / COVERT',value:950,a:'#67e4d7',b:'#294e70',bg:'#172f39'},{name:'Solar Fang',type:'KNIFE / COVERT',value:1400,a:'#ffe174',b:'#f15e45',bg:'#38251f'}];
let balance=2500,inventory=starterWeapons.map(x=>({...x})),selectedInput=null,selectedTarget=null,high=0,activity=[];
...
function saveState(){localStorage.setItem(accountKey,JSON.stringify({name:localStorage.getItem(accountKey+'_name'),balance,inventory,selectedInput,selectedTarget,high,activity}));}
function loadState(name){const saved=localStorage.getItem(accountKey);if(!saved)return false;try{const state=JSON.parse(saved);if(state.name!==name)return false;balance=state.balance??2500;inventory=state.inventory??starterWeapons.map(x=>({...x}));selectedInput=state.selectedInput??null;selectedTarget=state.selectedTarget??null;high=state.high??0;activity=state.activity??[];$('playerName').textContent=state.name;return true;}catch(error){localStorage.removeItem(accountKey);return false;}}
...
function draw(){
	$('inputs').innerHTML=inventory.map((x,i)=>`<div class="card ${selectedInput===i?'selected':''}" data-input="${i}">${art(x)}<div class="info"><strong>${x.name}</strong><small>${x.type} · ${x.value} cr</small></div></div>`).join('');
	...
	$('inventoryCount').textContent=inventory.length+' / 10 owned';
	...
	const input=inventory[selectedInput],target=targets[selectedTarget];
	if(input&&target){const chance=Math.max(13,Math.min(94,Math.round(input.value/target.value*86)+8));$('chance').textContent=chance+'%';$('hint').textContent='Risk cost: '+input.value+' credits';$('cost').textContent=input.value+' cr';$('run').disabled=balance<input.value;}
...
function showOutcome(win,text){$('outcomeCard').className='outcome-card '+(win?'win':'loss');$('outcomeTitle').textContent=win?'UPGRADE WON':'UPGRADE LOST';$('outcomeText').textContent=text;$('outcome').classList.add('show');}
$('closeOutcome').onclick=()=>{$('outcome').classList.remove('show');};
// Consume the selected weapon immediately; only a win adds the target skin back.
$('run').onclick=()=>{const input=inventory[selectedInput],target=targets[selectedTarget];if(!input||!target)return;const chance=Math.max(13,Math.min(94,Math.round(input.value/target.value*86)+8));inventory.splice(selectedInput,1);selectedInput=null;balance-=input.value;$('run').disabled=true;$('orb').style.animation='spin .55s linear infinite';$('resultTitle').textContent='Core is running...';$('resultText').textContent='Your old skin is locked in. Calculating the outcome.';saveState();setTimeout(()=>{const win=Math.random()*100<chance;$('orb').style.animation='';if(win){inventory.push({...target});balance+=target.value;high=Math.max(high,target.value);$('resultIcon').textContent='✦';$('resultTitle').textContent='Upgrade successful.';$('resultText').textContent=input.name+' was replaced by '+target.name+'.';$('resultValue').textContent='+'+target.value+' cr';addFeed('Won '+target.name,'+'+target.value+' cr',true);showOutcome(true,input.name+' became '+target.name+'.');}else{$('resultIcon').textContent='×';$('resultTitle').textContent='The core missed.';$('resultText').textContent=input.name+' was lost. No replacement skin was awarded.';$('resultValue').textContent='-'+input.value+' cr';addFeed('Lost '+input.name,'-'+input.value+' cr',false);showOutcome(false,input.name+' was lost. No replacement skin was awarded.');}draw();},700);};
$('accountName').value=localStorage.getItem(accountKey+'_name')||'';
	$('accountForm').onsubmit=event=>{event.preventDefault();const name=$('accountName').value.trim();if(!name)return;const returningPlayer=loadState(name);if(!returningPlayer){balance=2500;inventory=starterWeapons.map(x=>({...x}));selectedInput=null;selectedTarget=null;high=0;activity=[];}$('playerName').textContent=name;localStorage.setItem(accountKey+'_name',name);saveState();$('accountGate').classList.add('hidden');draw();};
